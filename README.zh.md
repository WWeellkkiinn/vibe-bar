...
<div align="center">
  <img src="VibeIsland.gif" alt="Vibe Island 演示" width="600">
</div>
...
2. 创建 `%LOCALAPPDATA%\VibeIsland\` 状态目录
3. 写入 `.python-path`（gitignored），供 `vibeisland.vbs` 读取
...
> 如果你已有其他 hook，install.py 只替换 Vibe Island 条目，不影响其他配置。
...
状态文件路径：`%LOCALAPPDATA%\VibeIsland\state.json`
